# 每运营商 access-token 密钥 / Per-operator access-token keys
...
This document defines per-operator access-token key isolation and the planned and emergency rotation procedures for the production security contract.
The core invariant is that production accepts only the strict `rgs-operators-v2` trust document, with independently scoped Ed25519 signing material and authenticated `kid`, signature, validity-window, issuer, audience, and operator-binding checks.
Repository validation does not prove that protected key generation, external signer integration, rollout, revocation, incident handling, or regulatory approval has occurred; those remain adopter-controlled external gates.
...
仍可伪造。运营商、采用方安全责任角色、监管/测试实验室与事件 owner 决定通知、token/会话撤销、证据保留
与恢复服务审批。
