...
本文定义采用方 AWS 目标环境的运行责任和最低证据。它不把 AWS 服务健康、HTTP 200 或备份任务
...
| 责任方 | 主要职责 | 不得单独执行 |
| --- | --- | --- |
| 个人项目维护者 | 代码、定义绑定、应用告警、幂等恢复、发布兼容性 | 修改正式 IAM/KMS、删除备份、跳过钱包对账 |
| 采用方云平台责任角色 | AWS 账号、网络、EKS、ALB、CloudFront、RDS、AMP/AMG/CloudWatch | 改写游戏定义、关闭应用完整性检查 |
| 采用方 SRE/值班责任角色 | 事件响应、变更窗口、容量、演练、复盘 | 手工把未知钱包结果标成成功/失败 |
| 采用方数据库责任角色 | RDS 参数、角色、迁移审批、PITR、恢复验证 | 把管理/迁移角色交给运行时 Pod |
| 采用方安全责任角色 | IAM/KMS、WAF、秘密轮换、日志审计、供应链与事件调查 | 在无恢复验证时删除旧 key/version |
| 采用方指定的外部运营集成责任角色 | 正式启动、钱包、审计、对账与最终业务接收 | 用 `local-operator` 代替正式服务 |
| 采用方业务责任角色 | SLO、RTO/RPO、维护窗口、风险接受与正式放量 | 用源码测试代替业务/运营验收 |
...
与 AWS 正式环境完全分离；清理本机环境不能影响 Tunnelblick、采用方网络或任何正式 AWS 资源。
