...
严格脚本会强制 `RGS_REQUIRE_POSTGRES_TESTS=1`，只运行以下十一项精确测试，并用
...
依次完成 `up`、两个并发幂等 `up`、`verify` 和十一项真实集成测试。模式就绪后才会继续
...
上述十一项 PostgreSQL conformance、race、runtime smoke 和单元 benchmark 证明正确性与回归边界，
...
数据库不健康或无法运行十一项测试时属于真实发布阻塞；不得用 mock、跳过或手工伪造
PASS 证据替代。
