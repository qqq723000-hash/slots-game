# Frontend host integration contract / 前端宿主集成契约
...
The operator host creates each RGS session, while the browser consumes a one-time `launchCode` and never issues, stores, or replays it. On timeout, failure, or committed-result recovery, the client disables betting, rejects late responses, clears the old handoff, and requests one fresh complete session through the versioned host event contract. Production use still requires the operator to validate the exact origin, source window, message shape, player and policy claims, signing path, relaunch behavior, and end-to-end recovery in its real host environment.
