# Operator integration guide / 运营商集成指南
...
This draft defines a neutral operator-to-RGS integration boundary whose normative HTTP shape is the repository OpenAPI contract. The operator owns player authentication, eligibility and responsible-gaming controls, jurisdiction policy, and the authoritative wallet ledger, while the RGS owns payable results, feature state, immutable definition binding, and durable resolution of one atomic wallet operation. Real-money acceptance remains prohibited until the exact deployment has the required licenses, independent RNG and mathematics certification, security assessment, tested wallet adapter, operator conformance, reconciliation, and operational approval.
