...
- Geometry comes from live original Primal Rampage launcher captures at the listed viewports, compared against the same local viewport after the responsive root has committed.
- Captured Play'n GO branding is reference-only. Release continues to use the approved local provider assets.
...
- Visual artwork and pointer target are separate contracts: original-sized artwork may be below 44 physical pixels, while the transparent pointer target remains at least 44×44 physical pixels.
...
- Mobile utility surfaces use the original translucent treatment; they must not become a near-opaque dark toolbar.
...
- Landscape phone/tablet uses the original left vertical rail. At 844×633 the rail is approximately x=22px, width=109px, with tab tops around 22/70/118px; content begins around x=181px.
