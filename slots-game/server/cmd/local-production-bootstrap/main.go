...
// English: The local-production-bootstrap command generates one-time initialization materials for native
// integration acceptance. This command only accepts an empty directory and will never overwrite existing files.
// The build result is LOCAL_TECHNICAL_PRODUCTION: it satisfies the runtime production configuration verification,
// but the approval reference only represents a user-authorized native technology deployment and makes no claims
// about external regulation, certification, or third-party rights.
package main
...
// English: localOperatorKeyDocument only references the protocol direction actually required by the native
// companion service. The private key direction and RGS configuration are strictly complementary to avoid misuse of
// identities after manual splicing at the deployment layer.
...
func prepareOutputDirectory(configured string) (string, error) {
	...
	// Bootstrap material must not be overwritten in place, or configuration and keys could come from different generations.
...
func generate(directory string, now time.Time) error {
	...
	// Use an independent key for each protocol direction so compromise of one service cannot forge other identities laterally.
	...
	// English: HMAC keys use standard Base64 required by the runtime loader; passwords and bearer tokens use RawURL
	// Base64 without command line and URL special characters for easy injection as container keys.
	...
	// 不能把 CA 私钥暴露给任何在线服务。
	// English: The private CA is only used for inter-service TLS on this machine; the deployment layer must be mounted
	// as a minimum file and cannot expose the CA private key to any online service.
...
// 均被明确排除在此事务之外。
// English: rotateLocalProductionDefinition only upgrades signed game definitions in existing local deployments.
// The defined approval authority remains stable; TLS materials, protocol keys, passwords, tokens, and persistence
// service data are explicitly excluded from this transaction.
...
// English: loadDefinitionGenerationForRotation maintains the strictness of regular production loaders. Only this
// offline migration command recognizes explicitly enumerated local predecessor versions; it verifies the
// predecessor version's specification summary and Ed25519 approval before allowing rotation.
...
// 转移不透明的证据引用。
// English: Known predecessor versions are eligible for all offline migrations into the repository only if the
// signed evidence is exactly consistent with the native self-authorized configuration generated by this bootstrap.
// This prevents mathematically unrelated or externally approved definitions from transferring opaque evidence
// references to code-owned targets simply because the game ID and key shape are the same.
...
// 此后方可开始新的轮换。
// English: recoverPendingDefinitionRotation makes an interrupted double file replacement recoverable. Fully
// committed targets are retained; older or mixed versions are restored from a restricted backup before a new
// rotation can begin.
...
func localProductionDefinition() game.Config {
	...
	// Pin the complete mathematics parameters supported by the current engine into a non-demo production identity, avoiding runtime dependence
	// on development defaults or environment branches. Version changes must regenerate the digest and v2 signed approval.
	return game.Config{
...
// English: addSharedAdmissionMaterial upgrades the local state directory before Valkey was introduced without
// rotating or copying any existing keys. Failed to close when the material package is incomplete, preventing
// interruption or manual migration from silently mixing different generations.
