...
// English: SignRequest applies the fixed RFC 9421 style configuration of this package and signs the exact set of
// components documented by fixedSignatureComponentSet. Query strings and other content types are intentionally
...
// English: Authenticate completes pure verification and returns the signature-authenticated identity, but does not
// write the nonce storage. The HTTP boundary can be used for admission based on trusted operators; only after
// successful admission, the random number is consumed to avoid being restricted by traffic and continuously
// creating persistent writes. The caller must then call ConsumeNonce before business side effects can be
// triggered.
...
// English: ConsumeNonce is the only persistence phase after authentication. It must be called before any business
// side effects, and relies on NonceStore's atomic semantics to let the same signature request go through all
// replicas at most once.
func (v *RequestVerifier) ConsumeNonce(ctx context.Context, verified VerifiedRequest) error {
	...
	// English: Private binding proves that the value is indeed generated by Authenticate of the same validator
	// instance, and that the public fields are not overwritten by the internal caller between admission and random
	// number consumption; the signature verification phase cannot be bypassed by manually constructing values.
	...
	// English: The application clock will accept the request before expires+skew at the latest; the PostgreSQL clock
	// may lead the application by an allowed deviation, so the random number must be kept until expires+2*skew to
	// cover the complete acceptance window.
...
// English: Verify is reserved for internal calls that do not require a separate admission phase; its security
// semantics are consistent with historical behavior.
