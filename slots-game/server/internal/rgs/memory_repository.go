...
// English: MemoryRepository is a reference implementation of the Repository contract that correctly handles
// concurrency. Locks are divided by session, allowing unrelated players to advance concurrently. It is used for
// testing and adapter consistency verification, and is not used as production persistent storage.
...
// English: RecoverySnapshot provides the same database global bounded lower bound semantics for memory contract
// adapters as PostgreSQL. It is only used for testing and consistency verification; production metrics are still
// generated by the PostgreSQL storage clock.
...
func (r *MemoryRepository) ScheduleWalletRecovery(
	...
	if disposition.ApplyNotSent {
		...
		// English: Only return economic APPLY budgets that can be proven not to have crossed the sending boundary.
		// RetryCount is a persistent scheduling pressure count that must be retained to expand the backoff, otherwise
		// persistent fusing will degenerate into a fixed short-period thermal cycle.
...
func claimMemoryWallet(
	...
	if action == WalletRecoveryApply {
		...
		// English: The durable successor of APPLY must first become a LOOKUP before passing execution to the external
		// caller.
...
// English: ValidateWalletReceipt Validates that the wallet response is an exact economic confirmation of a
// previously persisted command.
func ValidateWalletReceipt(command WalletRound, receipt WalletReceipt) error {
