...
const REVIEWED_INLINE_SCRUB_CSP_SOURCE = "'sha256-vUs+nbdxmdqOL3f/mZqTupLfHkYf373z+iYtj/+kHtM='";
const scriptTags = [...indexSource.matchAll(/<script\b[^>]*>[\s\S]*?<\/script>/gu)]
  .map((match) => match[0]);
const inlineScrubMatch = scriptTags[0]?.match(
  /^<script\s+id="launch-fragment-scrub">([\s\S]*?)<\/script>$/u,
);
const preflightMatch = scriptTags[1]?.match(
  /^<script\s+src="([^"]*browser-preflight\.js)"\s*><\/script>$/u,
);
const moduleMatch = scriptTags[2]?.match(
  /^<script\s+type="module"[^>]*\ssrc="([^"]+)"[^>]*><\/script>$/u,
);

if (scriptTags.length !== 3 || !inlineScrubMatch || !preflightMatch || !moduleMatch
  || indexSource.indexOf(inlineScrubMatch[0]) >= indexSource.indexOf(preflightMatch[0])
  || indexSource.indexOf(preflightMatch[0]) >= indexSource.indexOf(moduleMatch[0])) {
  throw new Error("生产 HTML 必须且只能依次执行内联片段清理、经典浏览器 preflight 和唯一模块入口");
}
if (/\bnomodule\b/u.test(preflightMatch[0])) {
  throw new Error("浏览器 preflight 必须在所有浏览器中先执行");
}

const inlineScrubSource = inlineScrubMatch[1];
...
const preflightPath = await confinedDistributionPath(preflightMatch[1]);
const modulePath = await confinedDistributionPath(moduleMatch[1]);
...
const manifest = JSON.parse(await readFile(resolve(distributionRoot, "release-manifest.json"), "utf8"));
if (!Array.isArray(manifest.files)
  || !manifest.files.some((entry) => entry?.path === "browser-preflight.js")) {
  throw new Error("生产发布清单没有绑定浏览器 preflight");
}

process.stdout.write(`生产浏览器 preflight 契约通过：内联清理已由 ${inlineScrubCspSource} 锁定，能力先检测，模块入口无静态应用依赖。\n`);

async function confinedDistributionPath(urlValue) {
