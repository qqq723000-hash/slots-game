...
function requireCanonicalJson(contents, approval, assets) {
  // 这是仓库审批生成器输出的编码。逐字节比较还能检测重复对象键， / English: This is the encoding output by the warehouse approval generator. Byte-by-byte comparison can also detect duplicate object keys.
  // 因为 JSON.parse 会在其他情况下将它们静默折叠为最后一个值。 / English: Because JSON.parse will silently collapse them to the last value in other cases.
 ...
 *
 * 英文 / English: Verify the accurate manifest generated by this network build with external approval provided by the key mount. The approval format is deliberately kept closed: schemaVersion 1, as well as status, approvalReference, jurisdiction, expiresAt and assets [{path, bytes, sha256}]; substitution of loose fields or in-repository placeholders for real authorization is prohibited.
 */
