...
/** GameReelLogicController.stopOutroHandler 为倍率揭示保留的制作时长。 */
export const PRIMAL_WILD_REVEAL_OUTRO_MS = 1_000;
/** GameReelLogicController.stopOutroHandler的三Rage激活保持。 */
...
export interface StopSequencerHooks {
  ...
  /** 核心屏障在第三次 STOPPED 冲击时触发，而反弹可能会继续。 */
  onAllReelsStopped?(event: AllReelsStoppedEvent): void;
  /** 第二个转轴撞击时，开始原生的第三轴悬念表现。 */
...
export class StopSequencer {
  ...
  /** 将 regularSpinTime 锚定到已接受的旋转请求，而不是其响应。 */
  ...
  requestFastForward(): boolean {
    // 当服务器响应仍在进行中时收到的 Continue 是正式排队的快速停止。因此，markSpinStart() 是主动圆形栅极，而不是 `presenting`。
    ...
    // 在用户按下 Continue 之前，悬念处理程序已经消耗了其自己的内部快速停止步骤。因此，下一个用户输入必须立即激活 SLOW 制动器；需要另一次单击比捕获的控制器晚一步。
  ...
  async present(
    ...
    try {
      ...
      // 响应迟到时，原始 getStopDelayOffset 只把第一个转轴的时间钳制到“当前时刻”。后续转轴仍保留 300ms 间隔。
      const firstPresentationDelay = Math.max(0, firstPlannedDelay - elapsedSinceSpin);
      // 每个计时器都从同一时间原点开始，因此结算动画可以重叠，而转轴启动仍遵循权威的从左到右计划。若依次等待每次结算，会错误地把 settleMs 累加到每个预定的转轴间隔中。
      ...
      await Promise.all(plan.map(async (step) => {
        ...
        // 悬念表现和转轴 2 的 SLOW 在转轴 1 进入 STOPPED 时开始，而不是在其回弹后，也不是在虚构的三秒释放延迟后。
        ...
        // 显式生产模式是固定的。注入的 NORMAL 稳定选项仍然可用于确定性单元测试。
        ...
        const impact = (async (): Promise<void> => {
          ...
          try {
            if (!allStoppedFired && impactedReels.size === plan.length) {
              ...
              // ALLSTOPPED 是转轴输入窗口的正式终点。随后制作好的 Wild/Rage 停轴收尾不可跳过，即使本次旋转已经快进。
              this.clearQuickStopReleaseTimer();
              this.spinStartedAtMs = null;
              ...
              // 官方停止结尾从第三个 STOPPED 开始，并与剩余的 NORMAL/FAST 反弹并行运行。
  ...
  private async presentPostStopActivation(
    ...
    // 一个/两个 Rage 未命中时没有符号负责停轴收尾，但其 SLOW 转轴仍拥有同一套原生悬念 `hide`。
    // 将结果和 Rage-collection 表现流程保留在该剪辑后面，以便路径永远不会在幸存的火框下方开始。
    if (!plan) {
    ...
    // SLOW 转轴会在独立悬念轨道完成 `hide` 前进入 STOPPED。官方停轴收尾会等待 333.333ms 片段，
    // 因此 SCATTER_FEATURE_ACTIVATE 永远不会在幸存的火框下渲染。
  ...
  private wait(ms: number, fastForwardSkippable = true): Promise<void> {
    ...
    return new Promise((resolve) => {
      ...
      const timer = setTimeout(() => {
        // 排队的快速停止会接管所有尚未启动的转轴。即使普通计时器先到期，任何转轴都不能在官方 spinStart+600ms 下限前制动；下限计时器会将它们一并释放。
