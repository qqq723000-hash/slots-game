...
/** 原始共享 Spine 包装器默认值 (`defaultCrossfadeTime=.15`)。 */
...
export function createSpineView(data: SpineData, options: SpineViewOptions = {}): Spine {
  ...
  // Spine 可以在*任何*更新期间交换附件，包括 Pixi 拥有的 autoUpdate 蜱。
  // 几张分派的 Primal 图集在不透明的黑色 RGB 页面 (`add/...`) 上保留附加光线，
  // 因此在 NORMAL 混合处留下一个新交换的插槽会在其他正确的闪电后面产生矩形黑色阴影。在适配器边界保持这一不变性，
  // 而不是依赖每个单独的庆祝活动、轮子、HUD 或角色调用者来记住更新后修复。
  ...
  // 在第一次显式更新或自动更新之前，还要覆盖设置姿势中已存在的附件。
  enforcePrimalRegionBlendModes(view, blendOptions);
  return view;
}
...
export function partitionPrimalAdditiveSlots(
  ...
  for (const slot of primalBlendSlots(view)) {
    ...
    // Pixi-Spine 可以在时间线清除其附件后保留最后一个 Sprite/Mesh 实例。
    // 永远不要仅仅因为它属于此通道而恢复陈旧的纹理：这样做会在动态过渡期间将完整的图集图块泄漏为明亮（或黑色）矩形。
    ...
    // 返回值描述活动附件，而不是休眠槽定义。因此，Q/K 正确报告零附加输出。
    if (attachment && renderable) visible += 1;
  }
  return visible;
}

/** 将单个 Spine 恢复为其普通的单通道渲染能力。 */
